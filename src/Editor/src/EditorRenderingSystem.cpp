...
namespace Aether {
    EditorRenderingSystem::EditorRenderingSystem(Renderer::RenderSurfaceHandle surface)
        : Systems::RenderingSystem(surface) { }

    std::optional<Renderer::CameraDesc> EditorRenderingSystem::ResolveCamera(Scene::Scene& scene, float aspect) {
        for (const auto& [entity, transform, camera] : scene.View<Components::Transform, Components::EditorCamera>().each()) {
